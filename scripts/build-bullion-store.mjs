...
const ig = document({
  ...
  <section class="ig-product-detail" data-product-detail hidden><div class="ig-shell"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a><span>/</span><a href="/ig/">Storefront</a><span>/</span><span data-detail-breadcrumb>Product</span></nav><div class="product-detail-layout"><div class="product-detail-image" data-detail-image></div><div class="product-detail-copy"><div data-detail-dealer></div><h1 data-detail-title></h1><p class="product-detail-price" data-detail-price></p><p data-detail-description></p><a class="button gold" data-detail-cta target="_blank" rel="sponsored nofollow noopener noreferrer">Shop Now</a><p class="affiliate-note">Purchases are completed securely on the dealer’s website. eStack may earn a commission at no additional cost to you.</p></div></div><div class="ig-heading related-products-heading"><span>Continue browsing</span><h2>Related products</h2></div><div class="ig-product-grid" data-related-products></div></div></section>
...
function categoryPage([slug, title, description, mark]) {
  ...
  return document({
    ...
    schema,
    main: `<section class="page-hero category-hero"><div class="store-shell"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a><span>/</span><a href="/shop/">Shop</a><span>/</span><span>${title}</span></nav><div class="category-hero-grid"><div class="category-title"><span class="hero-mark">${mark}</span><div><span class="eyebrow">Bullion collection</span><h1>${title}</h1><p>${description}</p></div></div><figure class="category-hero-image"><img src="/assets/estack-bullion-og.svg" alt="${title} bullion from verified eStack dealers" width="1200" height="630" decoding="async"></figure></div></div></section><section class="store-section"><div class="store-shell"><div class="section-head"><div><span class="eyebrow">Curated selection</span><h2>${title} marketplace</h2></div><p>Only a focused selection with verified dealer destinations appears here.</p></div><div class="product-grid" data-product-grid data-category="${slug}"></div><div class="empty-state" data-empty-state><span>${mark}</span><h3>Curated products are coming soon.</h3><p>We will publish products only after the title, dealer, image and destination are verified.</p><a class="button dark" href="/shop/">Browse all categories</a></div></div></section><section class="store-section pale"><div class="store-shell"><div class="section-head"><div><span class="eyebrow">Continue exploring</span><h2>Related collections</h2></div></div><div class="related-row">${related}</div></div></section>`
  });
}
...
.page-hero{background:linear-gradient(135deg,#080808,#1a160f);color:#fff;padding:78px 0 88px}.page-hero h1{font-size:clamp(44px,6vw,76px);letter-spacing:-.055em;max-width:900px}.page-hero p{font-size:18px;color:#bbb;max-width:720px}.breadcrumbs{display:flex;gap:10px;color:#aaa;font-size:13px;margin-bottom:55px}.breadcrumbs a:hover,.breadcrumbs a:focus-visible{color:var(--gold-light)}.category-hero-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:48px;align-items:center}.category-hero-image{margin:0;border-radius:24px;overflow:hidden;box-shadow:var(--shadow);border:1px solid rgba(234,209,154,.28)}.category-hero-image img{display:block;width:100%;height:auto}.category-grid{display:grid;grid-template-columns:1fr 1fr;gap:15px}.category-card{display:grid;grid-template-columns:auto 1fr auto;gap:20px;align-items:center;padding:24px;border:1px solid var(--line);border-radius:18px;transition:.2s}.category-card:hover,.category-card:focus-visible{background:#0b0b0b;color:#fff;border-color:#0b0b0b}.metal-mark{width:54px;height:54px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#ead59e,#a77723);color:#171006;font-weight:800}.category-card strong,.category-card small{display:block}.category-card strong{font:800 20px "Manrope"}.category-card small{color:var(--muted);margin-top:4px}.category-card:hover small{color:#aaa}.arrow{font-size:22px}.editorial-callout{display:flex;align-items:center;justify-content:space-between;gap:40px}.editorial-callout h2{font-size:clamp(28px,4vw,48px);max-width:850px}.category-title{display:flex;gap:30px;align-items:center}.hero-mark{flex:0 0 110px;height:110px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#f2dfad,#a67828);color:#181006;font:800 31px "Manrope"}.product-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}.product-card{border:1px solid var(--line);border-radius:20px;overflow:hidden;background:#fff}.product-image{aspect-ratio:4/3;background:#eee;display:grid;place-items:center}.product-image img{width:100%;height:100%;object-fit:cover}.product-body{padding:22px}.product-body h3{font-size:20px}.product-detail-link{display:block}.product-detail-link:hover h3,.product-detail-link:focus-visible h3{text-decoration:underline;text-decoration-color:var(--gold)}.product-description{color:var(--muted);font-size:14px;line-height:1.55}.product-meta{display:flex;justify-content:space-between;gap:10px;margin:12px 0;color:var(--muted)}.dealer-pill{font-size:11px;font-weight:800;padding:5px 9px;border-radius:999px;background:#111;color:var(--gold-light)}.price-placeholder{font-weight:800}.product-cta{display:block;text-align:center;padding:12px;border-radius:999px;background:#111;color:#fff;font-weight:800}.showroom-empty{padding:18px 0}.showroom-grid:empty{display:none}.empty-state{text-align:center;padding:72px 24px;border:1px dashed rgba(0,0,0,.2);border-radius:var(--radius);background:var(--paper)}.empty-state>span{display:grid;place-items:center;width:70px;height:70px;border-radius:50%;margin:0 auto 18px;background:#111;color:var(--gold-light);font-weight:800}.empty-state h3{font-size:28px}.empty-state p{color:var(--muted);max-width:600px;margin:12px auto 24px}.related-row{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.related-row a{display:flex;justify-content:space-between;padding:22px;border-bottom:1px solid #bcb7a9;font-weight:800}
.ig-product-detail{padding:54px 0 68px;background:var(--paper)}.ig-product-detail .breadcrumbs{margin-bottom:30px;color:#666}.product-detail-layout{display:grid;grid-template-columns:1fr 1fr;gap:42px;align-items:center}.product-detail-image{aspect-ratio:1;border-radius:24px;overflow:hidden;background:#e9e5da;display:grid;place-items:center}.product-detail-image img{width:100%;height:100%;object-fit:cover}.product-detail-copy h1{font-size:clamp(36px,6vw,60px);margin:16px 0}.product-detail-copy>p{color:#555}.product-detail-price{font-size:22px!important;color:#111!important;font-weight:800}.affiliate-note{font-size:12px}.related-products-heading{margin-top:56px}.ig-product-detail[hidden]{display:none}
